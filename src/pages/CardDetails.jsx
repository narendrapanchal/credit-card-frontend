...
const CardDetail = () => {
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
      <img src={cardData?.src} alt={cardData?.name} className="w-full h-48 object-cover rounded" />
      ...
      {cardData?.pros?.length>0&&<><h2 className="text-xl font-semibold mt-4">Pros:</h2>
      <ul className="list-disc list-inside">
        {cardData?.pros && cardData?.pros.map((pro, index) => (
          <li key={index} className="text-gray-700">{pro}</li>
        ))}
      </ul></>}

      {cardData?.cons?.length>0&&<><h2 className="text-xl font-semibold mt-4">Cons:</h2>
      <ul className="list-disc list-inside">
        {cardData?.cons && cardData?.cons.map((con, index) => (
