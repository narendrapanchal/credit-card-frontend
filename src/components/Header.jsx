  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        ...
        {
          headers: {
            Authorization: `Bearer ${login.token}`, // Pass the token in the Authorization header
          },
        }
      );
        ...
        <div className="mb-4">
          ...
          {pros.map((pro, index) => (
            <input
              ...
              className="w-full mb-2 p-2 border border-gray-300 rounded bg-slate-700 text-white"
            />
          ))}
